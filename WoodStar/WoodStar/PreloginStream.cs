...
namespace WoodStar
{
    ...
    public class StreamHeader
    {
        ...
        public QueryNotificationsHeader? QueryNotifications { get; init; }
        public TransactionDescriptorHeader? TransactionDescriptor { get; init; }
        public TraceActivityHeader? TraceActivity { get; init; }

        public int Length => 4 + (QueryNotifications?.Length ?? 0) + (TransactionDescriptor?.Length ?? 0) + (TraceActivity?.Length ?? 0);

        public void WriteToBuffer(byte[] sqlBatchBuffer)
        {
            // TODO: Think a better solution about all this offsets, perhaps span/memory as parameter rather than byte[]
            sqlBatchBuffer.WriteIntLittleEndian(TdsHeader.HeaderSize, Length);
            TransactionDescriptor?.WriteToBuffer(sqlBatchBuffer);
        }
    }

    public class QueryNotificationsHeader
    {
        public int Length => 0;
    }

    public class TransactionDescriptorHeader
    {
        public TransactionDescriptorHeader(ulong transactionDescriptor, int outstandingRequestCount)
        {
            TransactionDescriptor = transactionDescriptor;
            OutstandingRequestCount = outstandingRequestCount;
        }
        public int Length => 2 + 4 + 8 + 4;

        public ulong TransactionDescriptor { get; }
        public int OutstandingRequestCount { get; }
        public void WriteToBuffer(byte[] sqlBatchBuffer)
        {
            var offset = TdsHeader.HeaderSize + 4;
            sqlBatchBuffer.WriteIntLittleEndian(offset, Length);
            sqlBatchBuffer.WriteUnsignedShortLittleEndian(offset + 4, 2);
            sqlBatchBuffer.WriteUnsignedLongLittleEndian(offset + 6, TransactionDescriptor);
            sqlBatchBuffer.WriteIntLittleEndian(offset + 14, OutstandingRequestCount);
        }
    }

    public class TraceActivityHeader
    {
        public int Length => 0;
    }

    public class SqlBatchStream
    {
        public SqlBatchStream(StreamHeader streamHeader, string sql)
        {
        ...
        public int Length => StreamHeader.Length + Sql.Length * 2;

        public void WriteToBuffer(byte[] sqlBatchBuffer)
        {
            StreamHeader.WriteToBuffer(sqlBatchBuffer);

            Encoding.Unicode.GetBytes(Sql, 0, Sql.Length, sqlBatchBuffer, StreamHeader.Length + TdsHeader.HeaderSize);
        }
    }
}
